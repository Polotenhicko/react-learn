...
// вызов setState() объединит аргумент с текущим состоянием
// пример
...
// можно обновлять по отдельности с помощи отдельных вызовов setState()
// componentDidMount() {
...
// состояния обновляются поверхностно, поэтому вызов this.setState({comments}) оставляет this.state.posts нетронутым
// но полностью заменяет this.state.comments

// В иерархии компонентов ни родительский, ни дочерние компоненты не знают,
// задано ли состояние другого компонента.Также не важно, как был создан определённый компонент — с помощью функции или с помощью класса.
// состояние доступно только для самого компонента и скрыто от других

// компонент может передать своё состояние вниз по дереву в виде пропсов дочерних компонентов

// <FormattedDate date={this.state.date} />
// Компонент FormattedDate получает date через пропсы, но он не знает, откуда они взялись изначально — из состояния Clock,
//  пропсов Clock или просто JavaScript - выражения:
